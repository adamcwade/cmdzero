 ...
 * data-fui="<relative file>:<line>:<column>" in dev builds.
 * line is 1-based, column is 0-based — matching @babel/parser locs,
 * so the daemon can resolve a stamp back to the exact AST node.
 */
module.exports = function fastuiSourceStamp({ types: t }) {
  return {
    name: 'fastui-source-stamp',
    visitor: {
      JSXOpeningElement(nodePath, state) {
        ...
        if (
          node.attributes.some(
            (a) => t.isJSXAttribute(a) && a.name && a.name.name === 'data-fui'
          )
        )
          return;
        ...
        node.attributes.push(
          t.jsxAttribute(t.jsxIdentifier('data-fui'), t.stringLiteral(stamp))
        );
      },
    },
  };
